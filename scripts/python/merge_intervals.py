...
class Component(object):
    ...
    def bedpe(self):
        ...
        size = self.size
        return ("\t".join([chrom_left, str(start1), str(end1),chrom_right,str(start2), str(end2),str(score),str(size)]))
...
with open (args.bedpe_file_sort,"rb") as fichier:
    for line in fichier:
        ...
        for interaction_str in interactions:
            ...
            left = new_interval(intervalle_str[0])
            right = new_interval(intervalle_str[1])
