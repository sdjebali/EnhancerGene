...
def DHS_correlation (DHS1, DHS2):
    coef = scipy.stats.spearmanr(DHS1.log_counts,DHS2.log_counts)[0]
    pvalue = scipy.stats.spearmanr(DHS1.log_counts,DHS2.log_counts)[1]
    return str(coef),str(pvalue)
...
"end","ID","Pearson_corr","p_value"])) #Header line
    ...
    for line in f:
        ...
        while prom_DHS.is_within_neighbourhood(all_DHS[i].interval):
            if prom_DHS.interval != all_DHS[i].interval:
                (coef,pvalue) = DHS_correlation(prom_DHS,all_DHS[i])
                print("\t".join([str(prom_DHS),str(all_DHS[i]),coef,pvalue]))
            i += 1
            if i >= len(all_DHS):
                break
